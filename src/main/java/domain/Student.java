...
public class Student extends BasicStudent {
    ...
    private List<Tuple<String, Integer>> exams = new ArrayList<>();
    ...
    public JsonObject toJsonObject() {
        ...
        for (Tuple<String, Integer> exam : exams) {
            JsonPair examName = new JsonPair("course",
                    new JsonString(exam.key));
            JsonPair examMark = new JsonPair("mark",
                    new JsonNumber(exam.value));
            JsonPair examStatus = new JsonPair("passed",
                    new JsonBoolean(exam.value > 2));
            JsonObject jsonExam = new JsonObject(examName, examMark,
                    examStatus);
